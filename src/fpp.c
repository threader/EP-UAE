...
{
	return (munge24 (m68k_getpc (&regs)) & 0xFFF80000) == 0xF80000 && !currprefs.mmu_model;
}
...
STATIC_INLINE uae_u32 next_ilong_fpu ( struct regstruct *regs)
{
	if (currprefs.mmu_model)
		return next_ilong_mmu (regs);
	else if (currprefs.cpu_cycle_exact)
		return next_ilong_020ce (regs);
	else
		return next_ilong (regs);
}
STATIC_INLINE uae_u32 next_iword_fpu (struct regstruct *regs)
{
	if (currprefs.mmu_model)
		return next_iword_mmu (regs);
	else if (currprefs.cpu_cycle_exact)
		return next_iword_020ce (regs);
	else
		return next_iword (regs);
}
...
uae_u32 get_fpsr (const struct regstruct *regs)
{
...
STATIC_INLINE int get_fp_value (uae_u32 opcode, uae_u16 extra, fptype *src)
{
	...
	*src = regs.fp[(extra >> 10) & 7];
	...
	break;
    case 6:
			ad = get_disp_ea_020 (&regs, m68k_areg(&regs, reg), next_iword_fpu(&regs));
	break;
    case 7:
	switch (reg) {
	case 0:
			ad = (uae_s32) (uae_s16) next_iword_fpu (&regs);
	    break;
	case 1:
			ad = next_ilong_fpu (&regs);
	    break;
	case 2:
			ad = m68k_getpc (&regs);
			ad += (uae_s32) (uae_s16) next_iword_fpu (&regs);
...
STATIC_INLINE int put_fp_value (struct regstruct *regs, fptype value, uae_u32 opcode, uae_u16 extra)
{
	...
	break;
    case 5:
			ad = m68k_areg (regs, reg) + (uae_s32) (uae_s16) next_iword_fpu (regs);
	break;
    case 6:
			ad = get_disp_ea_020 (regs, m68k_areg (regs, reg), next_iword_fpu (regs));
	break;
...
void fpuop_dbcc (uae_u32 opcode, struct regstruct *regs, uae_u16 extra)
{
	...
	disp = (uae_s32) (uae_s16) next_iword_fpu (regs);
...
#endif
	if (fault_if_no_fpu (opcode, m68k_getpc(regs) - oldpc))
		return;
...
#endif
	if (fault_if_no_fpu (opcode, m68k_getpc (regs) - pc))
		return;
...
void fpuop_arithmetic (uae_u32 opcode, struct regstruct *regs, uae_u16 extra)
{
	...
	if (fault_if_no_fpu (opcode, 4))
	    ...
	    if ((extra & 0x2000) == 0) {
		if (extra & 0x1000) {
						regs->fpcr = next_ilong_fpu (regs);
						native_set_fpucw (regs->fpcr);
		}
		if (extra & 0x0800)
						set_fpsr (regs, next_ilong_fpu (regs));
		if (extra & 0x0400)
						regs->fpiar = next_ilong_fpu (regs);
	    }
